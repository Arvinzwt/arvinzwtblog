 ...
 */
export function getSortedPostsData(current = 1, size = 10) {
  // 读取posts目录下的所有文件名
  ...
  // 进行分页处理
  const paginatedPosts = _.chunk(sortedPosts, size);
  return {
    records: paginatedPosts[current - 1] || [],
...
export function getSortedTagData() {
  const { records } = getSortedPostsData(1, 1000);
